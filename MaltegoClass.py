...
class EntityTypes:
	...
	ownerEntity = "maltego.Alias"
	projectName = "default_icon"
	hash = "virustotal.Hash"
	threat = "virustotal.Threat"
	ioc = "virustotal.IOC"
	avcompany = "virustotal.AVCompany"
	...
	def parseArguments(self,argv):
		...
		if (len(argv) > 2):
			if (argv[2] is not None):
				mvars = argv[2].split('#');
				for x in range(0,len(mvars)):
					vars_values = mvars[x].split('=')
